...
- [x] **Phase 15c** — Parameterized compile-time queries + **true zero-boxing parameter binding** (Priority 1 #4). ✅ `CompiledQueryTranslator` translates `Query.Compile((ctx, p…) => ctx.Set<T>()…)` lambdas — `Where` (comparisons, `&&/||/!`, null checks, boolean columns), `OrderBy/ThenBy`, constant-or-parameter `Skip/Take`, terminals `ToList/First/Single/Count/Any` — into baked SQL + an ordered parameter-binding plan; the generator emits an interceptor that returns a delegate binding the lambda's typed params via `ITypedParameterSink.Add<T>` → `NpgsqlParameter<T>{TypedValue}` (no value-type boxing). Unsupported shapes → `Query.Compile`'s identity delegate → runtime. New Core `ITypedParameterSink` + executor `QueryBound/ExecuteScalarBound`; `VeloInterceptorSupport.Execute*Bound`. 2 generator-driver unit + 6 integration (intercepted parameterized Where/2-param/string-eq/Count/ordered-Take pagination with zero runtime translation; graceful runtime fallback). *Note:* one small closure allocation per invocation (the binder) — value types are never boxed, which is the #4 requirement. *Finding:* a C# interceptor's signature cannot receive a query's **closure-captured** values (they live in the `IQueryable`'s expression tree, not as method arguments), so parameterized predicates (`Where(x => x.C == localVar)`, variable `Skip/Take`) cannot be intercepted at the plain call site without boxing. The correct vehicle is **`Query.Compile<TCtx, T1, …>((ctx, p1) => …)`** where the parameters are explicit typed lambda arguments: the generator emits a delegate that builds SQL with `p1…` as bound parameters and creates **`NpgsqlParameter<T> { TypedValue = … }`** *directly in generated straight-line code* (no intermediate `object`/list → genuinely zero boxing). Requires: Roslyn predicate translation in `SymbolQueryTranslator` (comparisons, `&&/||`, member access, null checks); `Query.Compile` call-site interception returning the generated delegate; typed parameter emission. (A runtime typed-binding list cannot achieve zero boxing for a heterogeneous parameter set — only generated code can — so this lands with the codegen, not as standalone infra.) **Recommended as the first task on "continue".**
